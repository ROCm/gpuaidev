...
- Educational, step-by-step format: small focused code cells with markdown explaining what/why/expected output. Not one giant code dump. Hard limit: no single code cell over 100 lines (measured at review, not eyeballed); split anything 60-100 lines (see Step 2.6).
- Trusted, upstream sources only (NO personal images, forks, or repos). Everything the tutorial pulls (code, packages, Docker images, models, data) must come from official upstream or a recognized trusted namespace, never from an individual's personal account. Check and flag:
  - AMD-specific forks used where upstream supports ROCm: any `git clone` / `pip install` pointing at `github.com/ROCm/<mainstream-project>` (or another AMD fork) instead of the upstream repo.
  - Personal / non-official Docker images: the base image must be an official ROCm-ecosystem image (`rocm/*`, `vllm/*`, `lmsysorg/*`, or another recognized vendor namespace), pinned to a tag. A personal Docker Hub namespace (`<user>/<image>`), a raw registry host/IP, or `:latest` on an unofficial repo is not acceptable.
  - Personal / unofficial GitHub repos: `git clone` or `pip install git+https://...` must target the canonical upstream project or its official org, not an individual's account, a gist, or a non-upstream fork.
  - Non-canonical package sources: avoid a custom `--index-url`/`--extra-index-url` to a non-official index, a personal wheel URL, or installing from a personal fork instead of PyPI / the official ROCm index / the upstream project.
...
- One logical task per code cell where practical. Prefer separating: imports, configuration, model loading, data preparation, inference/training setup, execution, and result inspection.
- Hard cell-size limit (this is measured at review time, not eyeballed): NO single code cell may exceed 100 lines - a 100+ line cell is a code dump that will block the review, and a leading comment or docstring does NOT exempt it. Inlining an external module/helper file into one giant cell to satisfy the single-notebook rule (Step 2.4) is NOT acceptable; break it into logical, separately-explained cells (or fetch/generate it and show the reader what it contains). Any code cell in the 60-100 line range should be split into smaller logical steps. Before you submit, count the lines of every code cell (e.g. `len("".join(cell["source"]).splitlines())`) and split anything over these thresholds.
