...
9. Educational format and cell granularity (MEASURE EVERY CODE CELL - this check is mandatory, not optional). You MUST compute the line count of every code cell before judging this notebook; do not eyeball it. Parse the JSON and count lines per code cell (e.g. `len("".join(cell["source"]).splitlines())`). Then apply these gates:
   - **Blocking**: any single code cell over 100 lines. A 100+ line cell is a code dump, not a teaching step, regardless of whether it has a docstring or a leading comment. Inlining an external module into one giant cell to satisfy the single-notebook rule (Step 8 / Step 4.4) does NOT satisfy this gate - the module must be broken into logical, separately-explained cells (or genuinely fetched/generated with the reader shown what it contains). Report each offending cell by index and line count, and state which logical sub-steps it should be split into.
   - **Flag for reviewer**: any code cell 60-100 lines, or a notebook where more than ~25% of code cells exceed 60 lines. Report the count and the specific cell indices.
   - **Structural expectation**: one logical task per cell (imports / config / model loading / data prep / setup / execution / result inspection separated), each introduced by a markdown cell BEFORE the implementing code. A code cell with no preceding markdown explanation is a flag.
   Always output the measured distribution (total code cells, median lines, max lines, list of cells > 60 lines) in the verdict so the granularity finding is grounded in numbers, not impression. Do not pass this check by skimming; if you have not counted the cells, you have not done this check.
