...
2. AMD fork vs upstream (blocking if a mainstream project is pulled from an AMD/ROCm fork that upstream already supports on ROCm). Scan for `git clone`/`pip install`/URLs referencing `github.com/ROCm/<mainstream-project>` or other AMD-specific forks of major frameworks. Flag for reviewer judgement.
